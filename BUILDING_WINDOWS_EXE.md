...
- If GitHub Actions reports a lockfile/cache error, this workflow disables setup-node package-manager caching and uses `npm install` (not `npm ci`), so it does not require a committed `package-lock.json`.
